...
chock_cols = [c for c in df.columns if 'choque' in c]
if not chock_cols:
    raise ValueError("Coluna do tipo de choque da tempestade não encontrada.")
chock_col = chock_cols[0]
...
df[chock_col] = df[chock_col].apply(normalize_text)
...
df['choque_padrao'] = df[chock_col].map(lambda x: chock_map.get(x, x))

chock_order = [ 'icme', 'cir', 'sir']

#cálculo das frequências
results = {}
for chock in chock_order:
    subset = df[df['choque_padrao'] == chock]

    total = subset[ES_TYPES].sum().sum()

    if total == 0:
        results[chock] = {t: 0.0 for t in ES_TYPES}
    else:
        results[chock] = {t: (subset[t].sum() / total) * 100 for t in ES_TYPES}
...
ax.set_xticks(x)
ax.set_xticklabels(['ICME', 'CIR', 'SIR'], fontsize=11)
...
plt.tight_layout()
plt.savefig("fbEs_choque_empilhado.png", dpi=300)
print("Gráfico salvo como choque_empilhado.png")
plt.show()
